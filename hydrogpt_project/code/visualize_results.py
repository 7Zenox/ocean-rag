...
# -------------------------------------------------------------------
# Load per-method CSVs produced by run_baseline_eval.py
# -------------------------------------------------------------------
df_corr = pd.read_csv("results/Corrupted_results.csv")
df_b1 = pd.read_csv("results/Baseline1_results.csv")
df_b2 = pd.read_csv("results/Baseline2_results.csv")

# Build a combined DataFrame similar to the old structure
df = pd.DataFrame({
    "Corrupted_RMSE": df_corr["RMSE"],
    "Baseline1_RMSE": df_b1["RMSE"],
    "Baseline2_RMSE": df_b2["RMSE"],
})
...
ax.set_title(
    "Baseline Method Comparison\nBathymetric Depth Correction Performance",
    fontsize=14,
    fontweight="bold",
)
ax.set_ylim(0, max(rmse_mean) + max(rmse_std) + 10)
...
plt.suptitle(
    "Bathymetry Correction Example: Tile 01",
    fontsize=14,
    fontweight="bold",
    y=1.02,
)
